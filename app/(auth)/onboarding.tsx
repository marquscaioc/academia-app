    ...
    <SafeAreaView className="flex-1 bg-dark-400">
      <View className="flex-1 justify-center px-8 max-w-[500px] w-full self-center">
        <View className="mb-10">
          <Text className="text-xs font-bold text-lime-500 tracking-widest uppercase mb-3">
            Passo final
          </Text>
            ...
            return (
              <Pressable
                ...
                className={`rounded-3xl p-6 border-2 ${
                  isSelected
                    ? "bg-surface-elevated border-lime-500"
              ...
              >
                <View className="flex-row items-center gap-4 mb-4">
                  <View className={`w-14 h-14 rounded-2xl items-center justify-center ${
                    isSelected ? "bg-lime-500/20" : "bg-surface-elevated"
                  }`}>
                    <Text className="text-2xl">{role.icon}</Text>
                  </View>
                  <View className="flex-1">
                    <Text className={`text-xl font-black ${
                      isSelected ? "text-lime-500" : "text-text-primary"
                    }`}>
                      {role.label}
                    </Text>
                  ...
                  <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${
                    isSelected ? "border-lime-500 bg-lime-500" : "border-surface-border"
                  }`}>
                    {isSelected ? (
                ...
                <View className="flex-row flex-wrap gap-2">
                  {role.features.map((f) => (
                    <View key={f} className={`px-3 py-1.5 rounded-full ${
                      isSelected ? "bg-lime-500/10" : "bg-dark-300"
                    }`}>
                      <Text className={`text-xs font-medium ${
                        isSelected ? "text-lime-500" : "text-text-muted"
                      }`}>
                        {f}
                      </Text>
                    </View>
        ...
        <Pressable
          ...
          className={`rounded-2xl items-center mt-8 ${
            selectedRole ? "bg-lime-500 active:bg-lime-600" : "bg-surface-border"
          }`}
