    ...
    <Modal visible animationType="slide" transparent>
      <View className="flex-1 justify-end">
        ...
        <View className="bg-dark-200 border-t border-surface-border rounded-t-3xl px-6 pt-6 pb-10 max-h-[70%]">
          ...
          {!connections?.length ? (
            <Text className="text-sm text-text-muted text-center py-6" style={{ fontFamily: font.regular }}>Nenhum aluno conectado para conversar ainda.</Text>
          ) : (
            <ScrollView>
              {connections.map((c) => (
                <Pressable
        ...
        {isLoading ? (
          ...
          <View className="flex-1 items-center justify-center px-8 py-12">
            ...
            <Text className="text-sm text-text-secondary text-center mt-2.5 max-w-[300px] leading-5" style={{ fontFamily: font.regular }}>
              Converse diretamente com seus alunos. As conversas aparecerão aqui.
            </Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <Pressable
                  onPress={() => router.push(`/(trainer)/chat/${item.id}` as never)}
